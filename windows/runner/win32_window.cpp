...
bool Win32Window::Create(const std::wstring& title,
  ...
  window_width_ = Scale(size.width, scale_factor);
  window_height_ = Scale(size.height, scale_factor);

  // WS_EX_TOOLWINDOW: hide from taskbar
  // WS_POPUP: popup style window
...
bool Win32Window::Show() {
  // Show window near tray on first launch
  ShowWindowNearTray();
  return true;
}

void Win32Window::SetupTrayIcon() {
  if (tray_icon_created_ || !window_handle_) return;
  ...
  wcscpy_s(tray_icon_data_.szTip, L"TrayMonster");
...
void Win32Window::ToggleWindow() {
  // If hidden very recently by WM_ACTIVATE, treat tray click as "stay hidden"
  ULONGLONG now = GetTickCount64();
  if (!window_visible_ && (now - last_hide_tick_) < 300) {
    return;
  }
  if (window_visible_) {
    HideWindow();
...
void Win32Window::ShowWindowNearTray() {
  ...
  if (abd.uEdge == ABE_BOTTOM) {
    // Bottom taskbar
    x = workArea.right - window_width_;
    y = workArea.bottom - window_height_;
  ...
  SetWindowPos(window_handle_, HWND_TOPMOST, x, y,
               window_width_, window_height_,
...
void Win32Window::HideWindow() {
  ...
  last_hide_tick_ = GetTickCount64();
}
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    ...
    case WM_ACTIVATE:
      if (LOWORD(wparam) == WA_INACTIVE && window_visible_) {
